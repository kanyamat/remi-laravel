...
class GetMessageController extends Controller
{
    ...
    {         
            ...
            $httpClient = new CurlHTTPClient('88NWMVOkHLCf+cP6OdyisuAo7Px/5eh3i5x7YLevt3gHa7U844LlPvwKSOnHFbfS/M7cgw89i+hnLUKvs8R7QWPl5zzBrgWPobq1JC2SyaFVvL7qK/8A5v+NQcHTXoPRE1F9QvfNEmT7SIa01ZTROgdB04t89/1O/w1cDnyilFU=');
            $bot = new LINEBot($httpClient, array('channelSecret' => '92ad9ccc19fc2af57a34fc6130156f73'));
...
///ถามน้ำหนักทุกวันจันทร์            
            }elseif ($userMessage == 'น้ำหนักถูกต้อง' && $sequentsteps->seqcode == '1004' ) {
                  // $case = 7;
                  $seqcode = '0000';
                  $nextseqcode = '0000';
                  $sequentsteps = (new SqlController)->sequentsteps_seqcode($user);
...
//ถามทุกวันเกี่ยวกับการกินอาหาร/ออกกำลังกาย/วิตามิน
               ...
               if($userMessage == 'แนะนำเมนูอาหาร'|| $userMessage == 'คำถามที่ถามบ่อย'|| $userMessage == 'แนะนำการออกกำลังกาย'|| $userMessage == 'บันทึกข้อมูลคุณแม่'|| $userMessage == 'แนะนำการใช้งาน'){
                  ...
                  $nextseqcode = '0000';
                        ...
                        $record = (new SqlController)->tracker_cal($user);
                  $dinner = $record->dinner;
                  $dessert_din = $record->dessert_din;
                 $tracker= $dinner." ".$dessert_din ;
           
                 $da =  (new CalController)->cal_food($tracker);
                 $sum =  (new CalController)->cal_sum_food($tracker);
               
                 $record = (new SqlController)->tracker_cal($user);
                 $breakfast = $record->breakfast;
                 $lunch = $record->lunch;
                 $dessert_lu = $record->dessert_lu;
                 $tracker= $tracker." ".$breakfast." ".$lunch." ".$dessert_lu;
                 $sum_all =  (new CalController)->cal_sum_food($tracker);

                         if($sum_all == null) {
                     
                           $userMessage1 = "มื้อเย็นคุณแม่ทาน \n - \n \n"."แคลอรี่รวม: - kcal \n"."จำนวนแคลอรี่ทุกมื้อ: - kcal"; 
                    }elseif($sum_all !== null && $da ==null ){
                            
                            $comma_separated = implode("\n", $da);
                           $userMessage1 = "มื้อเย็นคุณแม่ทาน \n - \n \n"."แคลอรี่รวม: - kcal \n"."จำนวนแคลอรี่ทุกมื้อ:".$sum_all."kcal"; 
...
//////ถามตอนกลางวัน
                 ...
                 $record = (new SqlController)->tracker_cal($user);
                 $breakfast = $record->breakfast;
                 $tracker= $tracker." ".$breakfast ;
                 $sum_all =  (new CalController)->cal_sum_food($tracker);
                        
                    if($sum_all == null) {
                     
                           // $userMessage = "มื้อกลางวันคุณแม่ทาน \n - \n \n"."แคลอรี่รวม: - kcal \n"."จำนวนแคลอรี่ทุกมื้อ: - kcal"; 
                       $userMessage  = '😋';
                    }elseif($sum_all !== null && $da ==null ){
